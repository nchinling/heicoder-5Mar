...
const users = [
  ...
  function WelcomeMessageSingapore(props) {
    return <li>Majulah Singapura, {props.name}</li>
  }
  function WelcomeMessagesList() {
    return <ul>
      {
        users.map(user => (
          user.country === 'Singapore'? <WelcomeMessageSingapore key={user.name} name={user.name}  /> : <WelcomeMessage key={user.name} name={user.name} country={user.country} />
        ))
      }
    </ul>
  }
